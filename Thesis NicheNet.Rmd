...
filtered_de.genes <- list()

# Loop through the de.genes list
for (i in names(de.genes)) {
  # Filter the diffexp data frame based on criteria
  filtered_diffexp <- de.genes[[i]]$diffexp %>%
    filter((qval <= 0.05) & (abs(lfc) >= 1.5))
  
  # Store the filtered results in the new list
  filtered_de.genes[[i]] <- list(
    "diffexp" = filtered_diffexp,
    "from" = de.genes[[i]]$from,
    "name" = de.genes[[i]]$name,
    "type" = de.genes[[i]]$type
  )
}

# Names for the filtered_de.genes list
names(filtered_de.genes) <- names(de.genes)
  
# sanity check
str(filtered_de.genes)
tail(de.genes$EGF_24h$table)
  
DE_EGF_24 <- de.genes$EGF_24h$diffexp 
DE_EGF_48 <- de.genes$EGF_48h$diffexp  
DE_OSM_24 <- de.genes$OSM_24h$diffexp  
DE_OSM_48 <- de.genes$OSM_48h$diffexp  
DE_HGF_24 <- de.genes$HGF_24h$diffexp 
DE_HGF_48 <- de.genes$HGF_48h$diffexp 
```
...
settings <- de.genes %>% lapply(convert_expression_settings_evaluation)
...
#visualisation provided in the vignette
performances %>%
...
#visualisation individual points
# Sample labels for demonstration
scorelabels <- c(
  "auroc" = "AUROC",
  "aupr_corrected" = "AUPR (corrected)",
  "mean_rank_GST_log_pval" = "Mean Rank GST Log P-Value",
  "auc_iregulon_corrected" = "AUC iRegulon (corrected)",
  "pearson" = "Pearson",
  "spearman" = "Spearman"
)

## adapted visualisation for thesis
performances %>%
  mutate(model = "NicheNet v2") %>%
  ggplot() +
  ...
  theme_bw() +
  labs(title = "Transcriptional response prediction evaluation", color = "", x = "") +  
  theme(plot.title = element_text(size = 15, hjust = 0.5))

ggsave("performances_NicheNet.png", plot = last_plot(), width = 10, height = 8, dpi = 300)
...
## Adapted visualisation for thesis
metrics <- c("auroc", "aupr_corrected", "mean_rank_GST_log_pval", "auc_iregulon_corrected", "pearson", "spearman")
my_colors <- c("red", "blue", "green")

# Create a list of plots for specific metrics
plots <- lapply(metrics, function(metric) {
  p <- ggplot(ligand_importances, aes(x = setting, y = .data[[metric]])) +
    geom_boxplot() +
    geom_jitter(aes(color = factor(test_ligand)), width = 0.2, alpha = 0.7,  size = 1.5) +
    labs(title = metric,
         x = " ",
         y = metric) +
...
combined_plots <- wrap_plots(plots, ncol = 2) +
  plot_annotation(title = "Ligand activity prediction evaluation", theme = theme(plot.title = element_text(size = 24, hjust = 0.5))) +
  theme(legend.position = "bottom")  # Adjust legend margin

# Save the combined plots with the overall title in the middle and a single legend
ggsave("ligand_activity_NicheNet.png", plot = combined_plots, width = 13, height = 12, dpi = 300)
```
##### 24 ######
...
gr_network_LINCS_OSM_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_OSM_24_NO_RNAseq_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_HGF_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_HGF_24_NO_RNAseq_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_EGF_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_EGF_24_NO_RNAseq_grn.txt", header = TRUE, sep = ";")

targets.GRN_OSM_24 <- unique(gr_network_LINCS_OSM_24$to)
targets.GRN_EGF_24 <- unique(gr_network_LINCS_EGF_24$to)
targets.GRN_HGF_24 <- unique(gr_network_LINCS_HGF_24$to)

table(filtered_de.genes$EGF_24h$diffexp$gene %in% gr_network_LINCS_EGF_24$to)
table(filtered_de.genes$OSM_24h$diffexp$gene %in% gr_network_LINCS_OSM_24$to)
table(filtered_de.genes$HGF_24h$diffexp$gene %in% gr_network_LINCS_HGF_24$to)

length(unique(gr_network_LINCS_EGF_24$to)) - (table(filtered_de.genes$EGF_24h$diffexp$gene %in% gr_network_LINCS_EGF_24$to)["TRUE"])
length(unique(gr_network_LINCS_OSM_24$to)) - (table(filtered_de.genes$OSM_24h$diffexp$gene %in% gr_network_LINCS_OSM_24$to)["TRUE"])
length(unique(gr_network_LINCS_HGF_24$to)) - (table(filtered_de.genes$HGF_24h$diffexp$gene %in% gr_network_LINCS_HGF_24$to)["TRUE"])

length(targets.GRN_OSM_24)
length(targets.GRN_EGF_24)
length(targets.GRN_HGF_24)

length(intersect(targets.NN, targets.GRN_OSM_24))
length(intersect(targets.NN, targets.GRN_EGF_24))
length(intersect(targets.NN, targets.GRN_HGF_24))
...
new_gr_network_total_24 <- gr_network %>% bind_rows(gr_network_LINCS_OSM_24) %>% bind_rows(gr_network_LINCS_HGF_24) %>%
  bind_rows(gr_network_LINCS_EGF_24)
...
weighted_networks_total <- construct_weighted_networks(
  ...
  gr_network = new_gr_network_total_24, 
...
ggsave("performances_NicheNet_24h_ATAC_no_RNA.png", plot = last_plot(), width = 10, height = 8, dpi = 300)
...
# Replace infinite values with 10000
ligand_importances_ATAC <- ligand_importances_ATAC %>% mutate(across(ends_with("log_pval"), ~ifelse(is.infinite(.x), 10000, .x)))

evaluation_ligand_prediction_ATAC <- ligand_importances_ATAC$setting %>% unique() %>% lapply(function(x){x}) %>%
...
# Visualize some classification evaluation metrics showing the ligand activity prediction performance
evaluation_ligand_prediction_ATAC <- evaluation_ligand_prediction_ATAC %>% dplyr::select(-aupr, -sensitivity_roc, -specificity_roc, -pearson, -spearman, -mean_rank_GST_log_pval) %>% gather(key = scorename, value = scorevalue, auroc:aupr_corrected)
scorelabels <- c(auroc="AUROC", aupr_corrected="AUPR (corrected)")
scorerandom <- c(auroc=0.5, aupr_corrected=0) %>% data.frame(scorevalue=.) %>% rownames_to_column("scorename")

evaluation_ligand_prediction_ATAC %>%
 filter(importance_measure %in% c("auroc", "aupr_corrected", "mean_rank_GST_log_pval", "auc_iregulon_corrected", "pearson", "spearman")) %>%
  ggplot() +
  geom_violin(aes(importance_measure, scorevalue, group = importance_measure, fill = importance_measure)) +
  geom_boxplot(aes(importance_measure, scorevalue, group = importance_measure),width = 0.1) +
...
# Create a list of plots for specific metrics
plots <- lapply(metrics, function(metric) {
  p <- ggplot(ligand_importances_ATAC, aes(x = setting, y = .data[[metric]])) +
    geom_boxplot() +
    geom_jitter(aes(color = factor(test_ligand)), width = 0.2, alpha = 0.7,  size = 1.5) +
    labs(title = metric,
         x = " ",
...
# Save the combined plots with the overall title in the middle and a single legend
ggsave("ligand_activity_NicheNet_ATAC_24_NO_RNA.png", plot = combined_plots, width = 13, height = 12, dpi = 300)
```


```{r ligand-target-matrix construction, echo=TRUE}
#import one ligand GRN networks (Ruth generated from https://zenodo.org/record/7418451#.ZGNiqXZBx9O (ANANSE networks))
gr_network_LINCS_OSM_RNA_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_OSM_24_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_HGF_RNA_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_HGF_24_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_EGF_RNA_24 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_EGF_24_grn.txt", header = TRUE, sep = ";")

#look into overlap target genes and target genes NicheNet
targets.GRN_OSM_RNA_24 <- unique(gr_network_LINCS_OSM_RNA_24$to)
targets.GRN_EGF_RNA_24 <- unique(gr_network_LINCS_EGF_RNA_24$to)
targets.GRN_HGF_RNA_24 <- unique(gr_network_LINCS_HGF_RNA_24$to)

length(targets.GRN_OSM_RNA_24)
length(targets.GRN_EGF_RNA_24)
length(targets.GRN_HGF_RNA_24)

table(filtered_de.genes$EGF_24h$diffexp$gene %in% gr_network_LINCS_EGF_RNA_24$to)
table(filtered_de.genes$OSM_24h$diffexp$gene %in% gr_network_LINCS_OSM_RNA_24$to)
table(filtered_de.genes$HGF_24h$diffexp$gene %in% gr_network_LINCS_HGF_RNA_24$to)

length(unique(gr_network_LINCS_EGF_RNA_24$to)) - (table(filtered_de.genes$EGF_24h$diffexp$gene %in% gr_network_LINCS_EGF_RNA_24$to)["TRUE"])
length(unique(gr_network_LINCS_OSM_RNA_24$to)) - (table(filtered_de.genes$OSM_24h$diffexp$gene %in% gr_network_LINCS_OSM_RNA_24$to)["TRUE"])
length(unique(gr_network_LINCS_HGF_RNA_24$to)) - (table(filtered_de.genes$HGF_24h$diffexp$gene %in% gr_network_LINCS_HGF_RNA_24$to)["TRUE"])

length(intersect(targets.NN, targets.GRN_OSM_RNA_24))
length(intersect(targets.NN, targets.GRN_EGF_RNA_24))
length(intersect(targets.NN, targets.GRN_HGF_RNA_24))
...
#add LINCS GRN to default GRN NicheNet
new_gr_network_total_RNA <- gr_network %>% bind_rows(gr_network_LINCS_OSM_RNA_24) %>% bind_rows(gr_network_LINCS_HGF_RNA_24) %>%
  bind_rows(gr_network_LINCS_EGF_RNA_24)

# aggregate the individual data sources in a weighted manner to obtain a weighted integrated signaling network
weighted_networks_total_RNA <- construct_weighted_networks(
  lr_network = lr_network, 
  sig_network = sig_network, 
  gr_network = new_gr_network_total_RNA_24, 
  source_weights_df = new_source_weights_df %>%
    dplyr::select(source, avg_weight) %>%
    rename(weight = avg_weight))
...
ggsave("performances_NicheNet_ATAC_24_with_RNA.png", plot = last_plot(), width = 10, height = 8, dpi = 300)
...
# Save the combined plots with the overall title in the middle and a single legend
ggsave("ligand_activity_NicheNet_ATAC_24_with_RNA.png", plot = combined_plots, width = 13, height = 12, dpi = 300)
```
...
First run GRN.R, there the weighted networks and individual ligand-target matrices are constructed. Load here the GRN networks constructed with ANANANSE
In GRN.R are the weighted networks and individual ligand-target-matrices constructed.
```{r ligand-target-matrix construction 48 NO_RNA_seq, echo=TRUE}
#import one ligand GRN networks (Ruth generated from https://zenodo.org/record/7418451#.ZGNiqXZBx9O (ANANSE networks))
gr_network_LINCS_OSM_48 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_OSM_48_NO_RNAseq_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_HGF_48 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_HGF_48_NO_RNAseq_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_EGF_48 <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_EGF_48_NO_RNAseq_grn.txt", header = TRUE, sep = ";")

targets.GRN_OSM_48 <- unique(gr_network_LINCS_OSM_48$to)
targets.GRN_EGF_48 <- unique(gr_network_LINCS_EGF_48$to)
targets.GRN_HGF_48 <- unique(gr_network_LINCS_HGF_48$to)

length(filtered_de.genes$EGF_48h$diffexp$gene[(filtered_de.genes$EGF_48h$diffexp$gene %in% gr_network_LINCS_EGF_48$to)])
length(filtered_de.genes$OSM_48h$diffexp$gene[(filtered_de.genes$OSM_48h$diffexp$gene %in% gr_network_LINCS_OSM_48$to)])
length(filtered_de.genes$HGF_48h$diffexp$gene[(filtered_de.genes$HGF_48h$diffexp$gene %in% gr_network_LINCS_HGF_48$to)])

table(filtered_de.genes$EGF_48h$diffexp$gene %in% gr_network_LINCS_EGF_48$to)
table(filtered_de.genes$OSM_48h$diffexp$gene %in% gr_network_LINCS_OSM_48$to)
table(filtered_de.genes$HGF_48h$diffexp$gene %in% gr_network_LINCS_HGF_48$to)

length(unique(gr_network_LINCS_EGF_48$to)) - (table(filtered_de.genes$EGF_48h$diffexp$gene %in% gr_network_LINCS_EGF_48$to)["TRUE"])
length(unique(gr_network_LINCS_OSM_48$to)) - (table(filtered_de.genes$OSM_48h$diffexp$gene %in% gr_network_LINCS_OSM_48$to)["TRUE"])
length(unique(gr_network_LINCS_HGF_48$to)) - (table(filtered_de.genes$HGF_48h$diffexp$gene %in% gr_network_LINCS_HGF_48$to)["TRUE"])

length(targets.GRN_OSM_48)
length(targets.GRN_EGF_48)
length(targets.GRN_HGF_48)

length(intersect(targets.NN, targets.GRN_OSM_48))
length(intersect(targets.NN, targets.GRN_EGF_48))
length(intersect(targets.NN, targets.GRN_HGF_48))
...
plot(ligand_target_matrix[,"EGF"], ligand_target_matrix_ATAC_3[,"EGF"],
     xlab = "NicheNet default regulatory potentials",
     ylab = "Regulatory potentials with ATAC",
     main = "Regulatory potentials EGF 48 hour")

plot(ligand_target_matrix[,"HGF"], ligand_target_matrix_ATAC_3[,"HGF"], xlab = "NicheNet default regulatory potentials", ylab = "Regulatory potentials with ATAC", main = "Regulatory potentials HGF 48 hour")

plot(ligand_target_matrix[,"OSM"], ligand_target_matrix_ATAC_3[,"OSM"], xlab = "NicheNet default regulatory potentials", ylab = "Regulatory potentials with ATAC", main = "Regulatory potentials OSM 48 hour")
```

```{r ATAC performances 48}
# so some increase, but over the entire spectrum
...
ggsave("performances_NicheNet_ATAC_48_NO_RNA.png", plot = last_plot(), width = 10, height = 8, dpi = 300)
...
# Save the combined plots with the overall title in the middle and a single legend
ggsave("ligand_activity_NicheNet_ATAC_48_no_RNA.png", plot = combined_plots, width = 13, height = 12, dpi = 300)
```


```{r ligand-target-matrix construction 48 with RNA, echo=TRUE}
#import one ligand GRN networks (Ruth generated from https://zenodo.org/record/7418451#.ZGNiqXZBx9O (ANANSE networks))
gr_network_LINCS_OSM_RNA <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_OSM_48_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_HGF_RNA <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_HGF_48_grn.txt", header = TRUE, sep = ";")
gr_network_LINCS_EGF_RNA <- read.delim2("C:/Users/klara/MaStat/Thesis/GithubThesis/Thesis_NicheNet/Ananse_networks/GRN/LINCS_MCF10A_EGF_48_grn.txt", header = TRUE, sep = ";")
...
plot(ligand_target_matrix[,"EGF"], ligand_target_matrix_ATAC_4[,"EGF"], xlab = "NicheNet default regulatory potentials", ylab = "Regulatory potentials with ATAC", main = "Regulatory potentials EGF 48 hour")

plot(ligand_target_matrix[,"HGF"], ligand_target_matrix_ATAC_4[,"HGF"], xlab = "NicheNet default regulatory potentials", ylab = "Regulatory potentials with ATAC", main = "Regulatory potentials HGF 48 hour")



plot(ligand_target_matrix[,"OSM"], ligand_target_matrix_ATAC_4[,"OSM"], xlab = "NicheNet default regulatory potentials", ylab = "Regulatory potentials with ATAC", main = "Regulatory potentials OSM 48 hour")

```

```{r ATAC performances 48 with RNA}
# so some increase, but over the entire spectrum
...
ggsave("performances_NicheNet_ATAC_48_with_RNA.png", plot = last_plot(), width = 10, height = 8, dpi = 300)
...
# Save the combined plots with the overall title in the middle and a single legend
ggsave("ligand_activity_NicheNet_ATAC_48_with_RNA.png", plot = combined_plots, width = 13, height = 12, dpi = 300)
