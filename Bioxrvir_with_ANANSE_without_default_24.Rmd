---
title: "Bioxrvrir with ANANSE without default 24"
...
if(length(expression_settings_validation_DE_one) > 0){
  ...
  writexl::write_xlsx(performances_one, path = "NicheNet_output/performances_one_ANANSE_without_default_24.xlsx")
    
  # Visualize some classification evaluation metrics showing the target gene prediction performance
  performances_one = performances_one %>% dplyr::select(-aupr, -auc_iregulon, -pearson_log_pval, -spearman_log_pval,
  ...
  print(performances_one %>%
          ...
          theme_bw() +
          ggtitle("24h with ANANSE data only one ligand without default GRN")
  )
}
...
if(length(settings_one) > 0){
  ...
  writexl::write_xlsx(evaluation_ligand_prediction_one, path = "NicheNet_output/ligand_one_ANANSE_without_default_24.xlsx")
