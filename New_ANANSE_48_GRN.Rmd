...
if(length(expression_settings_validation_DE_one) > 0){
  ...
  #writexl::write_xlsx(performances_one, path = "NicheNet_output/performances_one_48.xlsx")
...
new_gr_network_total_original <- gr_network %>% bind_rows(gr_network_LINCS_OSM) %>% bind_rows(gr_network_LINCS_HGF) %>%
  bind_rows(gr_network_LINCS_EGF)

weighted_networks_total_original = construct_weighted_networks(lr_network = lr_network, sig_network = sig_network, gr_network = new_gr_network_total, source_weights_df = new_source_weights_df)

new_gr_network_total$source <- str_replace_all(new_gr_network_total$source, "LINCS_MCF10A_HGF_48_NO_RNAseq", "LINCS_MCF10A")
new_gr_network_total$source <- str_replace_all(new_gr_network_total$source, "LINCS_MCF10A_OSM_48_NO_RNAseq", "LINCS_MCF10A")
new_gr_network_total$source <- str_replace_all(new_gr_network_total$source, "LINCS_MCF10A_EGF_48_NO_RNAseq", "LINCS_MCF10A")
...
ligand_target_matrix_total_original <- construct_ligand_target_matrix(weighted_networks = weighted_networks_total_original, ligands = ligands)
...
if(length(expression_settings_validation_DE_one) > 0){
  ...
  performances_total_original = settings_one %>% lapply(evaluate_target_prediction, ligand_target_matrix_total_original) %>% bind_rows() 
  ...
  performances_total_original = performances_total_original %>% dplyr::select(-aupr, -auc_iregulon,-pearson_log_pval,-spearman_log_pval , -sensitivity_roc, -specificity_roc) %>% gather(key = scorename, value = scorevalue, auroc:spearman)
...
for(per in c(0, 0.25, 0.5, 0.75, 1)){
  ...
  new_gr_network$source <- str_replace_all(new_gr_network$source, "LINCS_MCF10A_HGF_48_NO_RNAseq", "LINCS_MCF10A")
  new_gr_network$source <- str_replace_all(new_gr_network$source, "LINCS_MCF10A_OSM_48_NO_RNAseq", "LINCS_MCF10A")
  new_gr_network$source <- str_replace_all(new_gr_network$source, "LINCS_MCF10A_EGF_48_NO_RNAseq", "LINCS_MCF10A")
...
```{r statsplots NicheNet ligands, echo=TRUE, error=TRUE, fig.width=10, fig.height = 10}
performances_NicheNet_ligands["condition"] <- "NicheNet" 
performances_total_original["condition"] <- "ATAC" 
perf <- performances_NicheNet_ligands %>% rbind(performances_total_original)
...
```{r statsplots NicheNet ligands, echo=TRUE, error=TRUE, fig.width=10, fig.height = 10}
performances_total_original["condition"] <- "NicheNet" 
performances_total["condition"] <- "ATAC" 
perf <- performances_NicheNet_ligands %>% rbind(performances_total)
