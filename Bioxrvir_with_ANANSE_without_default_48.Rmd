---
title: "ANANSE_without_default_48"
...
new_gr_network <- gr_network_LINCS_OSM %>% bind_rows(gr_network_LINCS_HGF) %>% bind_rows(gr_network_LINCS_EGF)
...
if(length(expression_settings_validation_DE_one) > 0){
  ...
  writexl::write_xlsx(performances_one, path = "NicheNet_output/performances_one_ANANSE_without_default_48.xlsx")
  ...
  print(performances_one %>%
          ...
          theme_bw() +
          ggtitle("48h with ANANSE data only one ligand without default GRN")
  )
}
...
if(length(settings_one) > 0){
  ...
  writexl::write_xlsx(evaluation_ligand_prediction_one, path = "NicheNet_output/ligand_one_ANANSE_without_default_48.xlsx")
