...
which(names_split[,3] == "EGF" & names_split[,4] == 24)
which(names_split[,3] == "HGF" & names_split[,4] == 24)
which(names_split[,3] == "OSM" & names_split[,4] == 24)
which(names_split[,3] == "TGFb" & names_split[,4] == 24)
which(names_split[,3] == "IFNg" & names_split[,4] == 24)
which(names_split[,3] == "BMP2" & names_split[,4] == 24)


list_24_1 <- list(matrices[[1]], matrices[[7]], 
                matrices[[11]], matrices[[13]], 
                matrices[[9]], matrices[[3]])  

list_24_2 <- list(matrices[[5]], matrices[[19]], 
                matrices[[23]], matrices[[25]], 
                matrices[[21]], matrices[[15]])

list_24_3 <- list(matrices[[17]], matrices[[32]], 
                matrices[[36]], matrices[[38]], 
                matrices[[34]], matrices[[28]])

df_24_1 <- Reduce(function(x, y) merge(x, y, all=TRUE), list_24_1) 
df_24_1[is.na(df_24_1)] = 0
df_24_1 <- df_24_1 %>% remove_rownames %>% column_to_rownames(var = "VALUE")

df_24_2 <- Reduce(function(x, y) merge(x, y, all=TRUE), list_24_2)  
df_24_2[is.na(df_24_2)] = 0
df_24_2 <- df_24_2 %>% remove_rownames %>% column_to_rownames(var = "VALUE")

df_24_3 <- Reduce(function(x, y) merge(x, y, all=TRUE), list_24_3)  
df_24_3[is.na(df_24_3)] = 0
df_24_3 <- df_24_3 %>% remove_rownames %>% column_to_rownames(var = "VALUE")
...
all_data <- list(df_24_1, df_24_2, df_24_3)
