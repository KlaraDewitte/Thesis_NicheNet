...
ggplot(rawdata_subset_48) +
  geom_histogram(aes(x = sid18), stat = "bin", bins = 200) +
  xlab("Raw expression counts") +
  ylab("Number of genes") 

ggplot(rawdata_subset_48) +
  geom_histogram(aes(x = sid111), stat = "bin", bins = 200) +
  xlab("Raw expression counts") +
  ylab("Number of genes") 

mean_counts <- apply(rawdata_subset_48[,1:3], 1, mean)       
variance_counts <- apply(rawdata_subset_48[,1:3], 1, var)
...
table(gr_network_LINCS_OSM$to %in% table1$DE_info$OSM$external_gene_name)
table(gr_network_LINCS_EGF$to %in% table1$DE_info$EGF$external_gene_name)
table(gr_network_LINCS_HGF$to %in% table1$DE_info$HGF$external_gene_name)
```
